...
from internvl.vla.datasets.rlds import traj_transforms, obs_transforms
    ...
    *,
    ...
    def restructure(traj):
        ...
        if language_key is not None:
            ...
            elif traj[language_key].dtype != tf.string:
                raise ValueError(
...
def apply_trajectory_transforms(
    ...
    *,
    ...
    if skip_unlabeled:
        if "language_instruction" not in dataset.element_spec["task"]:
            if "interleaved_instruction" not in dataset.element_spec["task"]:
                raise ValueError("skip_unlabeled=True but dataset does not have interleaved language labels.")
            else:
                pass
                # dataset = dataset.filter(lambda x: tf.math.reduce_any(x["task"]["interleaved_instruction"]["language_instruction"] != ""))
        else:
    ...
    # invalid_data = dataset.filter(lambda x: tf.reduce_any(tf.strings.length(x['observation']['image_primary']) == 0)).reduce(0, lambda x, _: x + 1)
    # assert invalid_data == 0, "Invalid data found after goal relabeling"
    ...
    if goal_relabeling_strategy is not None:
        ...
        # invalid_data = dataset.filter(lambda x: tf.reduce_all(tf.strings.length(x['observation']['image_primary']) == 0)).reduce(0, lambda x, _: x + 1)
        # assert invalid_data == 0, "Invalid data found after goal relabeling"
    ...
    # 将 observation 和 action 拆分成 window_size 的长度的窗口，在这种情况下，数据集 episode 个数会增加
    dataset = dataset.traj_map(
        partial(
...
def apply_frame_transforms(
    ...
    *,
    ...
    if train:
        print("-------------------before enter image augument")
    ...
    *,
    ...
    Args:
        dataset_kwargs_list: list of kwargs, each element of which is passed to `make_dataset_from_rlds`.
            ...
            `traj_read_threads`, respectively. 这个 kwargs 是从需要修改的 transforms.py 和 configs.py 中获得的。
