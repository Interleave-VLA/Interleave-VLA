"""
Main eval agent. Only for Interleaved Simpler for now.
...
class InterleavedEvalAgent(EvalAgent):
    def __init__(self, cfg):
        ...
        self.model = InterleavedPiZeroInference(cfg, use_ddp=False)
    ...
    def run(self):
        ...
        env_adapter.reset(instruction, env, obs)
        ...
        while 1:
            # infer action chunk
            inputs = self.env_adapter.preprocess(env, obs)
            ...
            if truncated:
                log.info(f"End of episode {cnt_episode}, prompt: {self.env_adapter._prompt}, success: {success}")
